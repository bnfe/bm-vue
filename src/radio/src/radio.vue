<template lang="html">
  <bm-cell v-if='radioType === "cell"' :disabled='radioDisabled' isLink>
    <bm-icon slot='description' v-if='isChecked' type="right" :theme='radioDisabled ? null : groupTheme'></bm-icon>
  ...
  computed: {
    model: {
      ...
      set(val) {
        ...
        if (this.isGroup) {
          this.dispatch('bmRadioGroup', 'input', val);
        } else {
          this.$emit('input', val);
    ...
    isGroup() {
      ...
      while (parent) {
        if (parent.$options.name !== 'bmRadioGroup') {
  ...
  methods: {
    onValueChange(event) {
      ...
      this.$nextTick(_ => { // eslint-disable-line no-unused-vars
        if (this.isGroup) {
          this.dispatch('bmRadioGroup', 'change', [this.model, event]);
        } else {
